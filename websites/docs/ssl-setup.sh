...
sudo mkdir -p /var/www/alientranslate/nginx_conf/ssl
...
if [ -f "/etc/letsencrypt/live/sefa.name.tr/fullchain.pem" ]; then
    ...
    sudo cp /etc/letsencrypt/live/sefa.name.tr/fullchain.pem /var/www/alientranslate/nginx_conf/ssl/cert.pem
    sudo cp /etc/letsencrypt/live/sefa.name.tr/privkey.pem /var/www/alientranslate/nginx_conf/ssl/key.pem
    
    # Set proper permissions
    sudo chmod 644 /var/www/alientranslate/nginx_conf/ssl/cert.pem
    sudo chmod 600 /var/www/alientranslate/nginx_conf/ssl/key.pem
    ...
    echo "   0 12 * * * /usr/bin/certbot renew --quiet && cd /var/www/alientranslate && docker-compose restart nginx"
else
