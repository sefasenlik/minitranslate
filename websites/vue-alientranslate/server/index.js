...
app.get('/api/releases', (req, res) => {
  try {
    ...
    // Read all files in the releases directory
    const files = fs.readdirSync(releasesDir);
    
    // Filter for ZIP files and get file details
    ...
    res.json(releaseFiles);
    
  } catch (error) {
    console.error('Error reading releases directory:', error);
    res.status(500).json({ error: 'Failed to retrieve release files' });
  }
});
