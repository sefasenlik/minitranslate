...
app.post('/admin/updateuser', requireAdminAuth, (req, res) => {
    const { oldToken, name, surname, email, token, usage } = req.body;
    if (!oldToken || !name || !surname || !email || !token) {
        return res.status(400).json({ error: 'Missing fields' });
    }
    
    const userIndex = users.findIndex(u => u.token === oldToken);
    if (userIndex === -1) {
        return res.status(404).json({ error: 'User not found' });
    }
    
    // Check if new token conflicts with existing user (excluding current user)
    const tokenConflict = users.find(u => u.token === token && u.token !== oldToken);
    if (tokenConflict) {
    ...
    const emailConflict = users.find(u => u.email === email && u.email !== users[userIndex].email);
    if (emailConflict) {
...
app.post('/admin/deleteuser', requireAdminAuth, (req, res) => {
